...
model = load_model("runs\\" + args.run_name + "\\models\\model_best.hdf5")
seeds = utils.get_seeds(args.num_levels, args.word_rnn, args.snaking, args.start_from_top) # get a list of seeds
...
for i in range(args.num_levels):
	...
	utils.generate_level_data(level_data, seed, model, args.word_rnn)

	int_array = [utils.onehot_to_int(level_data[t, :]) for t in range(level_data.shape[0])]
